...
ui <- fluidPage(
  ...
  sidebarLayout(position="left", fluid=TRUE,
                ...
                sidebarPanel(
                  h4("How is Airbnb really being used in and affecting the Melbourne neighbourhoods?"),
                  ...
                  h6("Depending on the room type, availability, and activity, an airbnb listing could be more like a hotel, disruptive for neighbours, taking away housing, and illegal."),
                  plotlyOutput("ListingsPlot", height = 200),
                  br(),
                  h6("Entire homes or apartments highly available year round,probably don't have the owner present, could be illegal and are displacing residents"),
                ...
                mainPanel(
                  ...
                  h6("Hosts with multiple listings are more likely to be running a business, are unlikely to be living in the property, and in violation of most short term
                     rental laws designed to protect residential housing"),
                  fluidRow(
...
server <- function( input, output, session) {
  ...
  output$nhdMap <- renderLeaflet({
    ...
    df$color = ifelse(df$room_type == "Private room", "orange" , 
                      ifelse(df$room_type == "Entire home/apt", "blue", "green"))
